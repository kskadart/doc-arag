...
async def process_upload_task(
    ...
    try:
        ...
        if not chunks:
            update_task_status(
                task_id, "failed", "No text content found in document",
                file_id=file_id
            )
            return
        
        # Step 4: Generate embeddings
        update_task_status(task_id, "processing", f"Generating embeddings for {len(chunks)} chunks")
        ...
        # Complete
        update_task_status(
            task_id, "completed", f"Successfully processed {filename}",
            file_id=file_id,
        ...
        update_task_status(
            task_id, "failed", f"Error: {str(e)}",
            file_id=file_id
        )


async def process_scraping_task(
    task_id: str,
    file_id: str,
    url: str
) -> None:
    ...
    try:
        ...
        if not chunks:
            update_task_status(
                task_id, "failed", "No text content found on page",
        ...
        # Complete
        update_task_status(
            task_id, "completed", f"Successfully processed {url}",
            file_id=file_id,
            url=url,
            chunks_count=len(chunks)
        )
...
async def process_embedding_task(
    ...
    """
    try:
        update_task_status(task_id, "processing", f"Processing embeddings for {file_id}")
        ...
        if not files:
            update_task_status(
                task_id, "failed", f"No files found for {file_id}",
                file_id=file_id
            )
            return
        ...
        if not chunks:
            update_task_status(
                task_id, "failed", "No text content found",
                file_id=file_id
            )
            return
        ...
        # Complete
        update_task_status(
            task_id, "completed", f"Successfully processed embeddings for {file_id}",
            file_id=file_id,
            chunks_count=len(chunks)
        )
