...
| 9. Дальнейший тюнинг | todo | | независимый judge (другая модель) + прогон на OpenRouter-моделях для сравнения; вычитка golden человеком; products-tariffs (4.31); tarif-010 — переформулировать; кандидаты: запрос-инструкция для Qwen3-Embedding (готово в ветке `feat/retrieval-tuning`, worktree `.claude/worktrees/tuning`, не влито — retrieval и так 98%), альтернативные источники в golden (`alt_sources`), rerank_top_k, judge-калибровка, сравнение с OpenRouter-моделями |
| 10. Диалоговые сессии: `session_id` в `/query`, `GET/DELETE /sessions/{id}`, Weaviate `ChatMessages` + TTL 7 дней, окно 6 + summary после 12, condense-промпт с историей; клиент шлёт id чата | done, влито | 2026-09-15 | doc-arag PR #13 в `feat/api-model-providers`, doc-arag-client PR #18 в `main`; live на локальных моделях: местоимения разрешаются, история переживает рестарт api, 14 ходов → summary, 12/12 контрольных без session_id; решение №11. При вливании в `feat/auth` эндпоинты сессий поставлены на `user_router`, владелец сессии — в backlog |
