...
| A1. Auth: `src/docarag/auth.py` (Remote-* → CurrentUser, `require_admin`), `GET /me`, `AUTH_MODE`, порты на 127.0.0.1 — ветка `feat/auth` (worktree `.claude/worktrees/auth`, от `feat/api-model-providers`); Authelia + Caddy — в `doc-arag-client` `feat/auth` | done (не влито) | 2026-09-15 | решение №11 |
