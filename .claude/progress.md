...
| 10. Диалоговые сессии (worktree `feat/chat-sessions`): `session_id` в `/query`, `GET/DELETE /sessions/{id}`, Weaviate `ChatMessages` + TTL 7 дней, окно 6 + summary после 12, condense-промпт с историей; клиент шлёт id чата | done (не закоммичено) | 2026-09-15 | ветка подтянута до `17967fd` (генерация = промпт v3 + сводка/история); 199 passed / 4 skipped, ruff+mypy чистые; live на локальных моделях: местоимения разрешаются («А как его подключить?» → «настройка статического IP»), история переживает рестарт api, DELETE идемпотентен, 14 ходов → summary на 8 сообщений (113 слов), 12/12 контрольных без session_id; ADR 11; клиент: `tsc --noEmit` и eslint чистые (Node 26 поставлен через brew 2026-09-15) |
