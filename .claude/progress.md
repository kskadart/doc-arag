...
| A1. Auth: `src/docarag/auth.py` (Remote-* + `X-Auth-Proxy-Secret` → CurrentUser), роутеры `user_router`/`admin_router`, `GET /me`, `AUTH_TRUSTED_HEADERS`/`AUTH_PROXY_SECRET`, порты на 127.0.0.1, `scripts/api_auth.py` — ветка `feat/auth` (worktree `.claude/worktrees/auth`, от `feat/api-model-providers`); Authelia + Caddy — в `doc-arag-client` `feat/auth` | done (не влито) | 2026-09-15 | решение №11 |
