...
## Прод (после запуска 2026-09-16)
- Нет мониторинга: ни внешнего health-чека, ни алерта на 403 от OpenRouter или падение прокси, ни бэкапа томов Weaviate и MinIO.
- Прокси — единственная точка отказа для моделей: при его недоступности прод замолкает.
- Пароль прокси прозвучал в переписке — сменить и обновить `OUTBOUND_HTTPS_PROXY`.
- Качество на проде проверено только смоуком из 15 вопросов; полный golden против прод-URL не гонялся.
- `scripts/load_corpus` на проде запускается через `docker cp` в контейнер api: в продовом образе нет каталога `scripts`.

## Интерфейс
- Чат не показывает поле ввода, пока не нажать «New Chat», хотя приветствие просит задать вопрос.
- Страница документов выводит сырые заголовки S3 (`X-Amz-*`, `Accept-Ranges`).
- Оператору показываются служебные «Rephrased Query» и «Confidence» — решить, нужно ли это в проде.
